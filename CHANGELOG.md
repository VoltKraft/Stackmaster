...
- Draft `deploy/compose.yaml` (modern Compose Spec) with all
  services commented.
...
- **ADR-0005** — Credential vault: **internal PostgreSQL vault** as
  ...
  delegated backends.
- **ADR-0006** — Auth: **local accounts** (env-var bootstrap admin,
  Argon2id, PATs, sessions) ship in v0.1 and carry the project
  through v0.8. **Native OIDC** (via `go-oidc`) is deferred to
  **v0.9**, deliberately the last major capability before v1.0,
  with **Authentik** as the primary conformance target (external,
  not bundled).
